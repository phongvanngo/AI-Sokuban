...
class Menu:
    ...
    def click(self, click_pos, window):
        ...
        if (
            x > self.new_game_txt_position[0]
            and x
            < self.new_game_txt_position[0] + self.new_game_txt_surface.get_width()
            and y > 300
            and y < 300 + self.new_game_txt_surface.get_height()
        ):
            sokoban = Game(window)
            sokoban.start()
        elif (
            x > self.load_game_txt_position[0]
            and x
            < self.load_game_txt_position[0] + self.load_game_txt_surface.get_width()
            and y > 370
            and y < 370 + self.load_game_txt_surface.get_height()
        ):
            sokoban = Game(window)
            sokoban.scores.load()
        elif (
            x > self.quit_game_txt_position[0]
            and x
            < self.quit_game_txt_position[0] + self.quit_game_txt_surface.get_width()
            and y > 440
            and y < 440 + self.quit_game_txt_surface.get_height()
        ):
            return False
    ...
    def render(self, window):
        ...
        self.new_game_txt_position = (
            (SOKOBAN.WINDOW_WIDTH / 2) - (self.new_game_txt_surface.get_width() / 2),
            300,
        )
        ...
        self.load_game_txt_position = (
            (SOKOBAN.WINDOW_WIDTH / 2) - (self.load_game_txt_surface.get_width() / 2),
            370,
        )
        ...
        self.quit_game_txt_position = (
            (SOKOBAN.WINDOW_WIDTH / 2) - (self.quit_game_txt_surface.get_width() / 2),
            440,
        )
...
def main():
    ...
    while run:
        ...
        pygame.draw.rect(
            window, SOKOBAN.WHITE, (0, 0, SOKOBAN.WINDOW_WIDTH, SOKOBAN.WINDOW_HEIGHT)
        )
        menu.render(window)
        pygame.display.flip()
